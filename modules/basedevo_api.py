...
class BasedApi(Requestor):
    ...
    def get_owned_nfts_number(self) -> list[int]:
        """
        ...
        except ConnectionError(429):
            logger.error('Have to sleep a bit')
            raise MakePause(60)
        ...
        nfts_owned = [nft_owned['metadata']['properties']['number'] for nft_owned in response['ownedNfts']]
        return nfts_owned
    ...
    def check_eligibility(self, nft_id: list[int] | int) -> list[int]:
        """
        Returns the list of nfts available for claim tokens
        :param nft_id: (list[int]|int) Can take the list of nft ids, of single id as argument
        :return: the list of nfts available for claim, e.g - [284574, 457484]
        """

        if type(nft_id) is int:
            nft_id = [nft_id]
    ...
    def send_message(self, nonce: int, nft_id: list[int] | int, signature: HexBytes, timestamp: int) -> bool:
        """
